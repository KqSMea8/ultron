 ...
 * http单接口查看配置模型
 */
export default class ModelSceneTask extends AModuleModel {
    public modelConfigScene;
    ...
    @observable public flags: boolean = false;
    ...
    public get vUrlTaskDetail() {
        const p = {
            planId: this.planId,
            taskId: this.taskId,
            type: this.modelConfigScene.type,
            planName: this.modelConfigScene.planName,
            pressureMode: this.modelConfigScene.pressureMode,
            businessCode: this.modelConfigScene.moreConfig.businessNum
        };
    ...
    constructor(query: IPlainObject = {}) {
        ...
        this.modelConfigScene = new ModelTestConfigScene(query.planId, query.planName);
    }
    ...
    public async loadDataList(): Promise<void> {
        ...
        // if (this.list.some((it) => it.item.status === 20)) {
        //     this.list.some((items): any => {
        //         items.item.status === 20 ? this.taskId = String(items.item.taskId) : '';
        //     });
        //     this.flags = true;
        // } else {
        //     this.flags = false;
